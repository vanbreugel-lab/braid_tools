...
- Production trigger path (see `TRIGGERED_VIDEO_SYSTEM.md`): ROS 1 machine's volume-trigger node publishes `Float64MultiArray [flag, obj_id, framenumber, t_wall, *stimulus]` → topic relay → `nodes/braid_trigger_adapter.py` converts to `BraidTrigger` (stamp = t_wall, braid-machine clock = same domain as acquire_stamp, so no cross-machine sync needed; stimulus preserved in metadata JSON) → video saver. `launch/braid_triggered_video_pipeline.launch.py` starts the whole ROS 2 side. Verified end-to-end with mock SSE servers (event captured the exact −1..+5 s window). NOTE for mock/test servers: SSE over HTTP/1.0 without chunked transfer encoding hangs `iter_content(chunk_size=None)` consumers — always use HTTP/1.1 + chunked.
